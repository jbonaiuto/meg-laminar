function all_subjects_erp(subjects, label, zero_event, woi, erp_type, varargin)
...
all_tpl = set(all_tpl, 'var', {'PAGETITLE','ZEROEVENT','WOI'}, {['ERP: All subjects: ' label], label, [erp_type num2str(woi(1)) '-' num2str(woi(2)) 'ms']});
...
for subject_idx=1:length(subjects)
    [subject_erps trial_times]=subject_erp(subjects(subject_idx), label, zero_event, woi, erp_type, 'data_dir', params.data_dir, 'url_prefix', params.url_prefix, 'time_limits', params.time_limits);
    keys=subject_erps.keys;
    for i=1:length(keys)
        cond_erps(keys{i})=[cond_erps(keys{i}) mean(subject_erps(keys{i}),2)];
    end
    subject_text=['<h2><a href="' params.url_prefix 'analysis/' subjects(subject_idx).subj_id '/erp/' label '_erp.html">Subject ' subjects(subject_idx).subj_id '</a></h2><table><tr><td><strong>All</strong></td><td><strong>Congruence</strong></td><td><strong>Coherence</strong></td><td><strong>Congruence x Coherence</strong></td></tr><tr><td><img src="' params.url_prefix 'analysis/' subjects(subject_idx).subj_id '/erp/' label '_all.png" width="100%"/></td><td><img src="' params.url_prefix 'analysis/' subjects(subject_idx).subj_id '/erp/' label '_congruence.png" width="100%"/></td><td><img src="' params.url_prefix 'analysis/' subjects(subject_idx).subj_id '/erp/' label '_coherence.png" width="100%"/></td><td><img src="' params.url_prefix 'analysis/' subjects(subject_idx).subj_id '/erp/' label '_congruence_coherence.png" width="100%"/></td></tr></table>'];
    
    all_tpl = set(all_tpl, 'var', {'SUBJECTOUT'}, {subject_text});
    all_tpl = parse(all_tpl,'subjects','subject',1);
end

img_path=fullfile('analysis','erp',[label '_all.png']);
...
img_path=fullfile('analysis','erp',[label '_coherence.png']);
...
img_path=fullfile('analysis','erp',[label '_congruence.png']);
...
img_path=fullfile('analysis','erp',[label '_congruence_coherence.png']);
