...
pial_wm_diff=fullfile(foi_dir,['pial-white.' comparison_name '.perc_change.shape.gii']);
...
pial_data=plot_surface_metric(subj_info, pial_surf, pial_comparison, view, 'ax', ax, 'limits', limits, 'threshold', params.threshold, 'plot', params.plot);

if params.plot || length(params.output_file)>0
    freezeColors;
    ax=subplot(1,4,2);
end
white_data=plot_surface_metric(subj_info, wm_surf, wm_comparison, view, 'ax', ax, 'limits', limits, 'threshold', params.threshold, 'plot', params.plot);
...
if abs(params.threshold)>0
    if length(params.white_pial_map)==0
        params.white_pial_map=map_white_to_pial(gifti(wm_surf), gifti(pial_surf));
    end
...
pial_white_data=plot_surface_metric(subj_info, pial_surf, pial_wm_comparison, view, 'ax', ax, 'mask', mask, 'plot', params.plot);
