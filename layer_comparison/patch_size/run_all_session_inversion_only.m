function run_all_session_inversion_only(subj_info, contrasts, varargin)

defaults = struct('data_dir', 'd:/pred_coding', 'inv_type', 'EBB',...
    'patch_size',0.4, 'surf_dir', '', 'mri_dir', '',... 
    'invert',true, 'extract', true, 'compare', true);  %define default values
params = struct(varargin{:});
for f = fieldnames(defaults)',
    if ~isfield(params, f{1}),
        params.(f{1}) = defaults.(f{1});
    end
end
if length(params.mri_dir)==0
    params.mri_dir=fullfile(params.data_dir,'mri');
end
if length(params.surf_dir)==0
    params.surf_dir=fullfile(params.data_dir,'surf');
end

addpath('D:\pred_coding\src\matlab\analysis\layer_comparison');

for i=1:length(contrasts)
    contrast=contrasts(i);
    for s_idx=1:length(subj_info.sessions)
        session_num=subj_info.sessions(s_idx);
        
        invert_pial(subj_info, session_num, contrast,...
            'data_dir', params.data_dir, 'inv_type', params.inv_type,...
            'patch_size',params.patch_size, 'surf_dir', params.surf_dir,...
            'mri_dir', params.mri_dir);
    
        invert_white(subj_info, session_num, contrast,...
            'data_dir', params.data_dir, 'inv_type', params.inv_type,...
            'patch_size',params.patch_size, 'surf_dir', params.surf_dir,...
            'mri_dir', params.mri_dir);
    end
end        

rmpath('D:\pred_coding\src\matlab\analysis\layer_comparison');