function invert_white(subj_info, session_num, contrast, varargin)

% Parse inputs
defaults = struct('data_dir', '/data/pred_coding', 'inv_type', 'EBB',...
    'patch_size',5, 'surf_dir', '', 'mri_dir', '', 'init', true,...
...
foi_dir=fullfile(data_dir, 'white_coreg', params.inv_type,...
...
if params.coreg
    ...
    matlabbatch{batch_idx}.spm.meeg.source.headmodel.meshing.meshes.custom.cortex = {fullfile(params.surf_dir,[subj_info.subj_id subj_info.birth_date '-synth'],'surf','ds_white.hires.deformed.surf.gii')};
